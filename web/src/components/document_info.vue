<template>
  <div class="docInfo">
    ...
    <div class="button_box">
      <el-button type="success" v-if="isCurrentUser" :disabled="disabled">同意</el-button>
      <el-button type="danger" v-if="isCurrentUser" :disabled="disabled">驳回</el-button>
...
export default {
  ...
  computed: {
    ...
    disabled() {
      if (this.document.status === "CANCEL") return true;
      else return false;
    }
  },
  methods: {
    async fetchNewDoc() {
      //获取该文档
      const res = await this.$axios.get("/web/document");
